...
const KEY = '21748328-14a002d5f3ca6b245a86a258f';
...
const getImages = (query) => {
  ...
  fetch(`https://pixabay.com/api/?key=${KEY}&q=${query}&image_type=photo&pretty=true`)
    ...
    .catch(err => showErrorMsg(true, 'Something went wrong! Please try again later.'))
}
...
const createSlider = () => {
  ...
  if (duration <= 0) {
    alert('Duration should be positive number only!');
    return;
  }
...
// Spinner
const toggleSpinner = (hideOrShow = false) => {
...
const showErrorMsg = (hideOrShow = false, msg = '') => {
